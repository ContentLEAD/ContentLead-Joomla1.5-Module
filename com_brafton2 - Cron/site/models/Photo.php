...
class Photo {
	...
	function __construct(){
		...
		$this->small = new PhotoInstance();
		$this->large = new PhotoInstance();
		$this->medium = new PhotoInstance();
	...
	public static function getPhotos($url){
		...
		foreach($photoItems as $photoNode){
			...
			foreach ($photoInstancesNode as $pi){
				...
				elseif ($type == "Small")$p->getSmall()->parsePhotoInstance($pi);
				elseif ($type == "Medium")$p->getMedium()->parsePhotoInstance($pi);
	...
	private function setSmall($small) {
		$this->small = $small;
	}
